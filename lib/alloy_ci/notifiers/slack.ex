defmodule AlloyCi.Notifiers.Slack do
  ...
  def send_notification(notification) do
    HTTPoison.post(
      config[:hook_url], build_payload(notification),
  ...
  defp base_notification_text(notification) do
    "The pipeline with ID: #{notification.content["pipeline"]["id"]} for the branch *#{notification.content["pipeline"]["ref"] |> String.replace("refs/heads/", "")}*"
  end
